 ...
 * CMD: mic
 **********************************************************************/
const char shell_help_mic[] = "<get/set> <register> <value>\n"
    "  get/set - The specific action to perform on the mic ADC on\n"
    "  register - The register address, in decimal, of the device that the action is being taken on\n"
    "  value - The value to set for a specific register during a set operation\n";
const char shell_help_summary_mic[] = "Performs an action on the ADAU1977 microphone ADC";

void shell_mic( SHELL_CONTEXT *ctx, int argc, char **argv )
{    
    ...
    if( argc == 3 ) {
        if ( strcmp(argv[1], "get") == 0 ) {
            u8RegisterAddr = strtol(argv[2], NULL, 0);
            if(read_adau1977(context->adau1977TwiHandle, ADAU1977_I2C_ADDR, u8RegisterAddr, au8Buffer, 1) == ADAU1977_SUCCESS) {
                printf( "Register address 0x%02x value is 0x%02x\n",  u8RegisterAddr, au8Buffer[0]);
                return;
            }
            else {
                printf( "Unable to read from register address 0x%02x\n",  u8RegisterAddr);
                return;
            }
        }
        else {
        ...
        if ( strcmp(argv[1], "set") == 0 ) {
            u8RegisterAddr = strtol(argv[2], NULL, 0);
            u8RegisterValue = strtol(argv[3], NULL, 0);
            au8Buffer[0] = u8RegisterAddr;
            au8Buffer[1] = u8RegisterValue;
            if(write_adau1977(context->adau1977TwiHandle, ADAU1977_I2C_ADDR, au8Buffer, 2) == ADAU1977_SUCCESS) {
                printf( "Wrote register address 0x%02x to value 0x%02x\n",  u8RegisterAddr, u8RegisterValue);
                return;
            }
            else {
                printf( "Unable to write to register address 0x%02x\n",  u8RegisterAddr);
                return;
            }
        }
    }
    else {
        printf( "Usage: <get/set> <register> <value>\n" );
        return;
    }   
}
...
void shell_format(SHELL_CONTEXT *ctx, int argc, char **argv)
{
    ...
    all = 0;
    if (argc > 1) {
        if (strcmp(argv[1], SPIFFS_VOL_NAME) == 0) {
            if (context->spiffsHandle == NULL) {
                printf("SPIFFS has not been configured for this project!\n");
                printf("Please check that SPIFFS is initialized in the startup task.\n");
                return;
            }
            sf = 1;
        } else {
            printf("Invalid drive name: \"%s\" does not match an existing partition.\n", argv[1]);
            printf("Call \"help format\" for options.\n");
            return;
        }
    } else {
        printf("Invalid argument. Please specify a valid partition. Call \"help format\" for options.\n");
        return;
    }
...
void shell_df( SHELL_CONTEXT *ctx, int argc, char **argv )
{
    ...
    if (argc > 1) {
        if (strcmp(argv[1], SPIFFS_VOL_NAME) == 0) {
            ...
            printf("Invalid drive name specified. Please call \"help fsck\" for options.\n");
            return;\
        }
    } else {
        if (context->spiffsHandle == NULL) {
...
int str2stream(char *stream, bool src)
{
    ...
    } else if (strcmp(stream, "mic") == 0) {
        return(src ? IPC_STREAMID_MIC_IN : IPC_STREAMID_UNKNOWN);
